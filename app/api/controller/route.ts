...
export async function POST(request: Request) {
    ...
    switch(req.c){
        ...
        case 'update': {
            const res = await col.updateOne({_id: new ObjectId(req.d._id)}, {$set: req.d})
            return new Response(JSON.stringify({ok:true}))
        }
        case 'delete': {
            const res = await col.deleteOne({_id: new ObjectId(req.d._id)})
            return new Response(JSON.stringify({ok:true}))
        }
        case 'read': {
            const res = await col.findOne({_id: new ObjectId(req.d._id)})
